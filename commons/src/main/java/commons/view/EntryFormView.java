...
public class EntryFormView {
    ...
    public EntryFormView(List<Integer> columnProportion) {
    ...
    private void putAttribute(String fieldName) {
        String nameToLowerCase = fieldName.toLowerCase(Locale.US);
        if (attributes.contains(nameToLowerCase)) {
            throw new IllegalArgumentException("Attribute with name '" + fieldName + "' already exists");
        }
        attributes.add(nameToLowerCase);
    }
